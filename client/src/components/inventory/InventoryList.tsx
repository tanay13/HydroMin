...
export default function InventoryList({ inventory, isLoading, onAddInventory }: InventoryListProps) {
  return (
    <Card>
      <CardHeader className="pb-2 pt-4 px-5 border-b border-neutral-200">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <CardTitle className="text-lg font-semibold text-neutral-500">Inventory Management</CardTitle>
          <Button 
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {isLoading ? (
            ...
            inventory.map((item) => {
              ...
              return (
                <Card key={item.id} className={`overflow-hidden ${isLow ? 'border-warning' : ''}`}>
                  <CardContent className="p-0">
                    <div className="p-5">
                      ...
                      <div className="flex items-center justify-between mb-3">
                        <h3 className="text-2xl font-semibold">{item.inStock.toLocaleString()}</h3>
                        {isLow && (
                      ...
                      <Progress 
                        value={percentage} 
                        className={`h-2.5 ${isLow ? 'bg-warning/20' : 'bg-neutral-200'}`}
                    ...
                    <div className="bg-neutral-50 p-4 border-t border-neutral-100">
                      <div className="grid grid-cols-2 gap-2 text-sm">
                        ...
                        <div>
                          ...
                          <div className="font-medium">{item.soldQuantity.toLocaleString()}</div>
                        </div>
                        {/* <div>
                        ...
                        <div>
                          ...
                          <div className="font-medium">{format(new Date(item.updatedAt), "MMM d")}</div>
                        </div>
                      </div>
                    </div>
                  </CardContent>
            ...
            <div className="col-span-3 text-center py-12">
              ...
              <p className="text-muted-foreground mb-4">You haven't added any inventory yet.</p>
              <Button 
                onClick={onAddInventory}
                className="bg-primary text-white hover:bg-secondary"
              >
