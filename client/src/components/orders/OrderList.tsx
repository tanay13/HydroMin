...
export default function OrderList({ orders, isLoading }: OrderListProps) {
	...
	const { mutate: updateStatus } = useMutation({
		...
		onSuccess: (data, variables) => {
			toast({
				title: "Order updated",
				description: `Order status changed to ${variables.newStatus.replace(
					"_",
					" "
				)}.`,
			});
										...
										<td className="px-4 py-4 text-sm">
											<div className="space-y-1">
												{order.items?.map((item, index) => (
													<div
														key={index}
														className="inline-flex items-center rounded-md bg-neutral-100 px-2 py-1 text-xs font-medium text-neutral-700 mr-1 mb-1"
													>
												...
												<div className="text-xs text-neutral-500 mt-1">
													...
													bottles
													<br />
													Sizes: {order.bottleSizes.split(",").join(", ")}
												</div>
											</div>
										</td>
										<td className="px-4 py-4">
